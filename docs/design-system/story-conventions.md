...
1.  **Extra exports are legal.** `check-contract.mts` asserts _declared states
    ⊆ story exports_, never the reverse. Case stories cannot break the
...
3.  **`ReducedMotion` documents a branch, or it documents its absence.**
    ...
    **`motion-reduce:animate-none` is inert on a Base UI popup surface — the
    ...
    both sides to a _single_ class of specificity — the data-attribute test is
    ...
    which sorts _after_ its counterpart and wins the same tie. Measured on
...
5.  **A `box-shadow` string is not a focus ring.** `KeyboardOrder` must show a
    ...
    - A Tailwind `ring-*` utility composes shadow _layers that are always
      present_, reading `rgba(0, 0, 0, 0) 0px 0px 0px 0px` when the ring is off.
    ...
    - **A treatment on an element that is not painted.** Base UI's slider puts a
      ...
      matches `:focus-visible`. Found on H2 `time-ruler` _after_ the helper below
    ...
    - **A permanent shadow reading as a ring.** This is the limit of the whole
      ...
      **Use both checks, because they answer different questions and can
      ...
      - J3 `explore-gallery`'s composer textarea gains a _colour_ on focus while
      ...
      - K3 `diff-review`'s vendored `Button` rests at `box-shadow: none`, so the
        differential flips on the transition's _first frame_, where all five ring
      ...
      **The differential works inside a tab walk too**, which this rule used to
      deny: read the _next_ stop's signature while focus is still on the previous
    ...
    non-zero alpha _and_ non-zero geometry, ignores an element that is not
...
7. **A pixel value is either dictated by a class or derived by the browser, and
   only the first can be pinned (D21).** Dictated: a computed style a class sets,
   a width a class fixes, a viewport the test moved. Derived: an intrinsic width,
   a `scrollWidth` over text, anything in `ch`, and anything a scrollbar
   participates in.

   This is not a style preference. The whole case-story program was verified on
   macOS and the gate runs on Linux; eleven story files were red on CI and green
   locally, and the same three measurements came out three different ways on
   three machines. Two traps inside it:

   - **A relationship can be just as unportable as an absolute.**
     `scrollWidth === clientWidth` and "this control is inside its panel" both
     failed because a classic scrollbar takes ~16px off a `clientWidth` that an
     overlay scrollbar does not. Re-anchor those to something the scrollbar does
     not move — `overflowX` rather than current overflow — instead of rephrasing
     them.
   - **A claim can be true on one platform only.** Where that happens, assert
     it conditionally and record the measurement, as `transport-controls` does
     for a timecode that overflows its field on macOS and exactly fills it on
     Linux. Pinning today's spill pins a defect; asserting a scroll that a tie
     makes impossible pins a platform.

   **Verify with `./scripts/linux-gate.sh`**, which runs the suite in the image
   CI uses, against your working tree, and takes a story path to run one file.
   It refuses to report a partial run: Chromium dying mid-suite otherwise looks
   like an improved failure count. A green run on a Mac has never said anything
   about CI.
