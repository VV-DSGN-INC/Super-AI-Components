    ...
    return (
      <div className="w-80 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm tabular-nums text-muted-foreground">{guidance.toFixed(1)}</span>
        </div>
        <Slider
          value={guidance}
          onValueChange={(value) => setGuidance(Array.isArray(value) ? value[0] : value)}
          min={1}
          max={20}
          step={0.5}
        />
        <p className="text-xs text-muted-foreground">Higher values follow the prompt more strictly.</p>
      </div>
    );
  },
};
  ...
  render: () => {
    ...
    return (
      <div className="w-80 space-y-3">
        ...
        <Slider
          value={steps}
          onValueChange={(value) => setSteps(Array.isArray(value) ? value[0] : value)}
          min={10}
          max={150}
          step={1}
        />
        <p className="text-xs text-muted-foreground">More steps = sharper detail, slower render.</p>
      </div>
    );
  },
};
    ...
    return (
      <div className="w-80 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm tabular-nums text-muted-foreground">{temp.toFixed(2)}</span>
        </div>
        <Slider
          value={temp}
          ...
          step={0.05}
        />
        <p className="text-xs text-muted-foreground">Lower is deterministic, higher is more experimental.</p>
      </div>
    );
  },
};
    ...
    return (
      <div className="w-80 space-y-3">
        ...
        <Slider
          ...
          step={64}
        />
        <p className="text-xs text-muted-foreground">Constrain the longest edge of generated images.</p>
      </div>
    );
  },
};
