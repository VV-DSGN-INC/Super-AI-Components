...
export const PreviewTileDocs: ComponentDocs = {
  ...
  accessibility: {
    ...
    screenReader: [
      ...
      "`labelPlacement` is a layout choice and no longer a naming one. `overlay` puts the label inside the frame, and `below` and `none` render a clipped copy inside it, so an interactive tile is named by its label whichever placement you pick. A tile with no `label` at all is still nameless — that part is yours.",
      "An interactive frame announces `aria-pressed` only when you pass `selected`, which is toggle-button semantics: right for a filter you switch on and off, wrong for a grid tile that simply opens. Omit `selected` and the tile announces as a plain button.",
