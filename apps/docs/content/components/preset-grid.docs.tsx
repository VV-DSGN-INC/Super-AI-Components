...
export const PresetGridDocs: ComponentDocs = {
  ...
  anatomy: [
    {
      slot: "preset-grid",
      note: "Root — the radiogroup (single-select) or group (multi-select) and the CSS grid live on the same element.",
    },
    {
      slot: "preset-grid-tile",
      note: 'One option: a role="radio"/"checkbox" button wrapping a preview-tile (A8) for the frame, ring, badge and overlay label.',
    },
    {
      slot: "preset-grid-swatch",
      note: "`palette` content only — the colour fill inside a tile. Decorative and aria-hidden; the tile's own label carries the accessible name.",
    },
    {
      slot: "preset-grid-see-more",
      note: "The overflow affordance — a tile in the grid, in the same role container, never a link rendered below it.",
    },
  ],
  usage:
