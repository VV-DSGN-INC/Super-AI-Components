...
- **The a11y exclusion list may only shrink, never grow.** Adding a file to silence a new failure defeats the gate; fix the component instead. → `a11y-baseline.md`. The same rule holds for `story-coverage.baseline.json`: regenerate it with `pnpm story-coverage:baseline`, which refuses to grow it. → `story-conventions.md`
