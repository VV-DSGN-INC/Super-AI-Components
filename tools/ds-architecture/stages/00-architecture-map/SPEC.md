...
- **Putting the empty-`scopeRoles` check in the JSON Schema.** `minProperties` would have
  ...
  _can_ tell: it read the config, and the config scopes nothing. That is exit `1`.
