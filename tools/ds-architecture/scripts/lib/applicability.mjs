...
/** A claim id is `NN.n`, optionally carrying a single lowercase suffix letter
 *  for a SUB-claim: `01.4c` is the coverage half of `01.4`, split off because a
 *  probe can settle whether a contrast test exists but not whether it covers
 *  every axis cell. The suffix earns its place by keeping the pair adjacent and
 *  visibly related — renumbering it to `01.7` would file a claim *about* `01.4`
 *  at the far end of the list.
 *
 *  Validated rather than trusted, because the whole point of declaring the list
 *  is to cross-check it against what a probe emits — and a malformed id can
 *  never match, so it would silently report the stage's own claim as never
 *  emitted. Widening the grammar preserves that: `1.4`, `01.`, `01.4cc` and
 *  `abc` are all still rejected. */
const CLAIM_ID_RE = /^\d\d\.\d+[a-z]?$/
