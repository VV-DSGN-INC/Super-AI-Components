...
function readFrontmatterList(acceptancePath, key) {
  ...
  const dashed = frontmatter[1].match(new RegExp(String.raw`^${key}:\s*\r?\n((?:\s*-\s*\S+\r?\n?)+)`, "m"));
...
export function readAppliesTo(acceptancePath) {
  ...
  if (values.length === 0) {
    throw new Error(
      `${acceptancePath}: appliesTo is empty — a stage that applies to no profile never runs and never reports`,
    );
  }
...
export function readClaims(acceptancePath) {
  ...
  if (values.length === 0) {
    throw new Error(
      `${acceptancePath}: claims is empty — a stage that declares no claims can never be cross-checked against its probe`,
    );
  }
