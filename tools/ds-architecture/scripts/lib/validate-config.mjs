...
export function validateAgainst(schema, value, where = "") {
  ...
  if (schema.type === "array") {
    ...
    if (schema.items) {
      value.forEach((item, i) => errors.push(...validateAgainst(schema.items, item, `${where}[${i}]`)));
    }
    return errors;
  }
  ...
  if (schema.type === "object") {
    ...
    for (const [key, child] of Object.entries(value)) {
      ...
      const pattern = Object.entries(schema.patternProperties ?? {}).find(([re]) => new RegExp(re).test(key));
      if (pattern) {
