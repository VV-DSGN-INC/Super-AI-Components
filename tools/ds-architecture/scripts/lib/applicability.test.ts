...
describe("readClaims", () => {
  ...
  it("accepts a sub-claim id like 01.4c, the coverage half of 01.4", () => {
    expect(readClaims(acceptanceWith("---\nclaims: [01.4, 01.4c]\n---\n"))).toEqual([
      "01.4",
      "01.4c",
    ])
  ...
  it("still rejects the near-misses the suffix could have let through", () => {
    for (const bad of ["1.4", "01.", "01.4cc", "01.4C", "abc"]) {
      expect(() => readClaims(acceptanceWith(`---\nclaims: [00.1, ${bad}]\n---\n`)), bad).toThrow(
        /malformed/
      )
    }
