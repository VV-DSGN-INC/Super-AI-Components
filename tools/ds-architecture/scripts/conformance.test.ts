...
describe("runConformance", () => {
  it("reports every claim met on the conformant fixture", async () => {
    // Scoped to stage 00, because this is stage 00's fixture and a stage's
    // fixture is judged by that stage's claims. Unscoped, every stage added to
    // the ladder would break every earlier stage's fixture — an O(n^2) coupling
    // whose only escape is fixtures quietly satisfying stages they know nothing
    // about, which is how a fixture stops testing the one thing it was built for.
    const out = await runConformance(path.join(F, "conformant"), { stage: "00" })
  ...
  it("does not discover the throwing fixture in a normal run", async () => {
    const out = await runConformance(path.join(F, "conformant"))
    // Discovery of stages/__throwing__ surfaces as exit 2 carrying its
    // deliberate reason. Asserting "not 2" rather than "0" keeps this test about
    // DISCOVERY: now that the ladder holds stages this stage-00 fixture does not
    // satisfy, exit 1 is the correct normal outcome, and pinning 0 would make
    // the test fail for a reason it was never asking about.
    expect(out.summary.exitCode).not.toBe(2)
    expect(out.summary.reason ?? "").not.toMatch(/deliberate/)
  })
